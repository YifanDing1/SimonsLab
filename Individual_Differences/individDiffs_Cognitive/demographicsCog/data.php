<?php
include('../../php_include/dbcreds_IndDiffsCog.php'); // Database connection file
...
try {
    ...
    // First, update end_time in metaDataCog if subject_code and end_time exist
    if (isset($data_array['subject_code']) && isset($data_array['end_time'])){
        $update_sql = "UPDATE metaDataCog SET end_time = :end_time WHERE subject_code = :subject_code";
    ...
    // Get all column names from the demographicsCog table
    $stmt = $conn->prepare("SHOW COLUMNS FROM `demographicsCog`");
    $stmt->execute();
    $col_names = array();
    ...
    // Prepare INSERT statement for demographicsCog
    $columns = [];
    $values = [];
    $placeholders = [];
    ...
    $sql = "INSERT INTO `demographicsCog` (" . implode(", ", $columns) . ")
            VALUES (" . implode(", ", $placeholders) . ")";

    // Log the SQL query for debugging
    error_log("SQL Query for demographicsCog: " . $sql);
